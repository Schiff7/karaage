...
以上代码通过默认构造函数生成了`A`类的一个实例，并对成员函数`bar`进行了调用。实例生成的过程中，没有生成一份新的`bar`函数的拷贝，调用过程可以等价为以下的伪代码：
...
利用这种方式生成对象会存在一个问题：上面代码中生成的`a`和`b`是两个独立的实体。在类模板实现的面向对象中所有对实例方法的调用都是调用类方法本身，而此处`a`和`b`的`bar`方法虽然完全一样，但是却是各自占用了一份内存资源。
